...
public class RegisterProductSaga {
    ...
    public Mono<UUID> registerProductCategory(RegisterProductCategoryCommand cmd, ExecutionContext ctx) {
        return cmd.getId() != null
                ? Mono.<UUID>empty().doFirst(() -> ctx.putVariable(CTX_CATEGORY_ID, cmd.getId()))
                : commandBus.send(cmd).doOnNext(categoryId -> ctx.putVariable(CTX_CATEGORY_ID, categoryId));
    }
    ...
    public Mono<UUID> registerProduct(RegisterProductInfoCommand cmd, ExecutionContext ctx) {
        return commandBus.send(cmd.withCategoryId(ctx.getVariableAs(CTX_CATEGORY_ID, UUID.class)))
                .doOnNext(productId -> {
                    ctx.putVariable(CTX_PRODUCT_ID, productId);
                    ctx.putVariable(CTX_DISTRIBUTOR_ID, cmd.getDistributorId());
                });
    }

    public Mono<Void> removeProduct(UUID productId, ExecutionContext ctx) {
    ...
    public Mono<UUID> registerLendingType(RegisterLendingTypeCommand cmd, ExecutionContext ctx) {
        return cmd.getId() != null
                ? Mono.<UUID>empty().doFirst(() -> ctx.putVariable(CTX_LENDING_TYPE_ID, cmd.getId()))
                :commandBus.send(cmd)
                .doOnNext(lendingTypeId -> ctx.putVariable(CTX_LENDING_TYPE_ID, lendingTypeId));
    }

    public Mono<Void> removeLendingType(UUID lendingTypeId, ExecutionContext ctx) {
    ...
    public Mono<UUID> registerLendingConfiguration(RegisterLendingConfigurationCommand cmd, ExecutionContext ctx) {
        return cmd.getId() != null
                ? Mono.<UUID>empty().doFirst(() -> ctx.putVariable(CTX_LENDING_CONFIGURATION_ID, cmd.getId()))
                : commandBus.send(cmd
                ...
                .doOnNext(lendingConfigurationId -> ctx.putVariable(CTX_LENDING_CONFIGURATION_ID, lendingConfigurationId));
    }

    public Mono<Void> removeLendingConfiguration(UUID lendingConfigurationId, ExecutionContext ctx) {
    ...
    public Mono<UUID> registerLeasingContract(RegisterLeasingContractCommand cmd, ExecutionContext ctx) {
        return cmd.getId() != null
                ? Mono.<UUID>empty().doFirst(() -> ctx.putVariable(CTX_LEASING_CONTRACT_ID, cmd.getId()))
                : commandBus.send(cmd
                ...
                .doOnNext(leasingContractId -> ctx.putVariable(CTX_LEASING_CONTRACT_ID, leasingContractId));
    }

    public Mono<Void> removeLeasingContract(UUID leasingContractId, ExecutionContext ctx) {
    ...
    public Mono<UUID> registerShipment(RegisterShipmentCommand cmd, ExecutionContext ctx) {
        return cmd.getId() != null
                ? Mono.<UUID>empty().doFirst(() -> ctx.putVariable(CTX_SHIPMENT_ID, cmd.getId()))
                : commandBus.send(cmd
                        .withLeasingContractId(ctx.getVariableAs(CTX_LEASING_CONTRACT_ID, UUID.class))
                        .withProductId(ctx.getVariableAs(CTX_PRODUCT_ID, UUID.class)))
                .doOnNext(shipmentId -> ctx.putVariable(CTX_SHIPMENT_ID, shipmentId));
    }
